...
int main(){
    ...
    cin >> N >> M;
    vector<vector<int>> bill(N);
    vector<vector<int>> shelter(M);

    rep(i,N){
        bill[i].resize(3);
        cin >> bill[i][0] >> bill[i][1] >> bill[i][2];
    }

    rep(i, M)
    {
        shelter[i].resize(3);
        cin >> shelter[i][0] >> shelter[i][1] >> shelter[i][2];
    }    
    ...
    {
        total_flow += bill[i][2];
        minimize_cost::add_edge(graph, 0, i + 1, bill[i][2],0);

        rep(j, M)
        {
            int distance = abs(bill[i][0] - shelter[j][0]) + abs(bill[i][1] - shelter[j][1]);

            minimize_cost::add_edge(graph, i+1, 1 + N + j + 1, shelter[j][2], distance);
        }                
    }

    rep(j,M){
        minimize_cost::add_edge(graph, 1 + N + j + 1,N+M+1, shelter[j][2], 0);
    }

    cout << minimize_cost::min_cost_flow(graph, 0, N + M +1, total_flow);
}
