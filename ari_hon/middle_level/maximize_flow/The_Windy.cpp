...
int main(){
    ...
    rep(i, N){
        minimize_cost::add_edge(graph,s,i,1,0);
    }
    ...
    rep(j,M){
        rep(k,N){
            ...
            minimize_cost::add_edge(graph,N + j * N + k, t, 1, 0);
            rep(i, N)
            {
                minimize_cost::add_edge(graph, i, N + j * N + k, 1, (k + 1) * Z[i][j]);
            }
        }        
    }
        
    cout << (double)minimize_cost::min_cost_flow(graph, s,t,N)/N;
}
