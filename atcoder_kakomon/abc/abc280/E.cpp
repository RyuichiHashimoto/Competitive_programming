...
void search(int N, mint prob2, mint prob1) {
    if (used[N]) return;
    ...
    if (N == 1) {
        ...
        dp[2] = prob2 * 1 + prob1 * 2;
        return;
    }

    search(N - 2, prob2, prob1);
    search(N - 1, prob2, prob1);
    dp[N] = dp[N - 2] * (prob2) + dp[N - 1] * (prob1) + 1;
    used[N] = true;
}
